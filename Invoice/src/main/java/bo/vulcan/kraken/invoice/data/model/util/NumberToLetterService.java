...
public class NumberToLetterService {
    ...
    public static String convert(BigDecimal value, String currency, boolean uppercase) {

        String numero = MyRoundNumberUtils.roundTwoDecimal(value).toString();
        ...
        if (Pattern.matches("\\d{1,9},\\d{1,2}", numero)) {
            ...
            if (uppercase) {
                return (literal + parte_decimal).toUpperCase() + currency;
            } else {
                return (literal + parte_decimal);
            }
        } else {//error, no se puede convertir
            System.err.println("Error no se puede convertir");
            return null;
        }
    }
